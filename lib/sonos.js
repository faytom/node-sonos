...
const optionsFromSpotifyUri = function (uri, title, region) {
  ...
  if (uri.startsWith('spotify:track:')) {
    ...
    const radioTitle = (title === undefined) ? 'Artist Radio' : title;
    return {
      uri: spotifyUri,
      ...
      if ((metadata) && (metadata[0].length > 0) && metadata[0] !== 'NOT_IMPLEMENTED') {
        return (new xml2js.Parser()).parseString(metadata, (err, data) => {
          ...
          if (track.albumArtURI) {
            track.albumArtURI = track.albumArtURI.startsWith('http') ? track.albumArtURI : 'http://' + self.host + ':' + self.port + track.albumArtURI;
          } else {
  ...
  getZoneInfo(callback) {
    ...
    this.request(this.options.endpoints.device, action, body, 'u:GetZoneInfoResponse', (err, data) => {
      ...
      for (const d in data[0]) {
        if (Object.prototype.hasOwnProperty.call(data[0], d) && d !== '$') {
          output[d] = data[0][d][0];
        }
      }
    ...
    request({
      ...
      (new xml2js.Parser()).parseString(body, (err, json) => {
        ...
        for (const d in json.root.device[0]) {
          if (Object.prototype.hasOwnProperty.call(json.root.device[0], d)) {
