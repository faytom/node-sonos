...
var Search = function Search (options) {
  ...
  self.startSearching = function(){
    ...
    var PLAYER_SEARCH = str2ab(['M-SEARCH * HTTP/1.1',
    ...
    var sendDiscover = function () {
      ['239.255.255.250', '255.255.255.255'].map(function (addr) {
        chrome.sockets.udp.send(self.socket, PLAYER_SEARCH, addr, 1900, function(result) {
          // console.log('==send sonos searching==')
          if (result.resultCode != 0) {
            chrome.sockets.udp.close(self.socket);
            self.onClose()
          }
        })
      })
    ...
    chrome.sockets.udp.create(function(createInfo) {
      ...
      chrome.sockets.udp.bind(self.socket, '0.0.0.0', 0, function(result) {
        if (result < 0) {
        } else {
          chrome.sockets.udp.setBroadcast(self.socket, true, function(result){
            if (result < 0) {
            } else {
              sendDiscover()
            }
  ...
  chrome.sockets.udp.onReceive.addListener(function(info) {
    // console.log("监听到返回数据 info.socketId: ", info.socketId, self.socket);
    if (info.socketId === self.socket) {
      self.receiveListener(info);
    }
  })
...
Search.prototype.destroy = function (callback) {
  ...
  chrome.sockets.udp.close(this.socket, callback)
}
